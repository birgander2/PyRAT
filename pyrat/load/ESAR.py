...
class ESAR(pyrat.ImportWorker):
    ...
    :param filename: The filename of the SLC file to import.
    :type array: string
    ...
    def reader(self, *args, **kwargs):
        ...
        if os.path.isdir(self.filename):
            ...
            files = glob.glob(os.path.join(self.filename,'*_ch*'+productname+'.dat'))
            files += glob.glob(os.path.join(self.filename+"/*/",'*_ch*'+productname+'.dat'))
        ...
        #----------------------------------------------------------------
            ...
            logging.error("File not found: " + ESAR.filename)
            return
...
def esar(*args, **kwargs):
    return ESAR(*args, **kwargs).run(**kwargs)
