...
class Rat(PyRat.ExportWorker):
    ...
    def writer(self, array, *args, **kwargs):
        srat(self.filename, np.squeeze(np.vstack(array)))
        return True
...
#-------------------------------------------------------------------------      
def srat(filename,array):
    """
    Writes a np ndarray into a RAT file
    """
    File = RatFile(filename)
    File.write(array)
    ...
    def read(self, **kwargs):
        if self.version == 2.0:
            ...
            if 'block' in kwargs:
                ...
                arr = np.ndarray(shape=blockshape[::-1],dtype=self.dtype)
...
#--------------------------------------------------------------------------   
    def read_header(self):      
        if self.version == 2.0:
            ...
            if self.xdrflag == 1: e='>'
            lun = open(self.filename,'rb')      
            self.ndim = np.fromfile(file=lun, dtype=e+"i4",count=1)
            self.dim  = np.fromfile(file=lun, dtype=e+"i4",count=self.ndim)  
            self.var  = np.fromfile(file=lun, dtype=e+"i4",count=1)
            self.typ  = np.fromfile(file=lun, dtype=e+"i4",count=1)
            foo  = np.fromfile(file=lun, dtype=e+"i4",count=1)
            foo  = np.fromfile(file=lun, dtype=e+"i4",count=1)
            foo  = np.fromfile(file=lun, dtype=e+"i4",count=1)
            if self.xdrflag == 1: foo  = np.fromfile(file=lun, dtype=e+"i4",count=1)
            self.info  = string.rstrip(np.fromfile(file=lun, dtype="B",count=80).tostring())
            lun.close()
        else:
            print "ERROR: RAT version not supported" 
            raise IOError
