...
class PluginManager(object):
	...
	def _getPluginNameAndModuleFromStream(self, infoFileObject, candidate_infofile="<buffered info>"):
		...
		if PLUGIN_NAME_FORBIDEN_STRING in name:
			logging.debug("Plugin name contains forbiden character: %s (in '%s')" % (PLUGIN_NAME_FORBIDEN_STRING,candidate_infofile))
			return (None, None, None)
	...
	def _gatherCorePluginInfo(self, directory, filename):
		...
		name,moduleName,config_parser = self._getPluginNameAndModuleFromStream(open(candidate_infofile),candidate_infofile)
	...
	def gatherBasicPluginInfo(self, directory,filename):
		...
		if config_parser.has_section("Documentation"):
			...
			if config_parser.has_option("Documentation","Description"):
                ...
                if config_parser.has_section("Menu"):
                        if config_parser.has_option("Menu","Menu"):
                                plugin_info.menu = config_parser.get("Menu", "Menu").strip()
                        if config_parser.has_option("Menu","Entry"): 
                                plugin_info.entry = config_parser.get("Menu", "Entry").strip()
                        if config_parser.has_option("Menu","Order"):
                                plugin_info.order = config_parser.get("Menu", "Order").strip()
                        if config_parser.has_option("Menu","Shortcut"):
                                plugin_info.shortcut = config_parser.get("Menu", "Shortcut").strip()
                        if config_parser.has_option("Menu","Tooltip"):
                                plugin_info.tooltip = config_parser.get("Menu", "Tooltip").strip()
		return plugin_info
